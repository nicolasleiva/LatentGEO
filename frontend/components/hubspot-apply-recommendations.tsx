...
export default function HubSpotApplyRecommendations({ auditId }: { auditId: string }) {
    const [recommendations, setRecommendations] = useState<HubSpotRecommendation[]>([])
    ...
    const selectAll = (type: 'all' | 'auto' | 'none') => {
        if (type === 'none') {
            ...
            setSelectedRecs(new Set(
                recommendations.filter(r => r.auto_fixable).map(r => r.id)
            ))
        }
    }
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-3">
                {recommendations.map((rec) => {
                    ...
                    const wasApplied = results.find(r => r.page_id === rec.hubspot_page_id)
                                ...
                                <div className="flex items-start justify-between">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            />
                                            <CardTitle className="text-lg">{rec.page_title}</CardTitle>
                                            ...
                                            {wasApplied && (
                                                <Badge
                                                    variant={wasApplied.success ? "default" : "destructive"}
                                                >
                                                    {wasApplied.success ? (
                                                        <>
