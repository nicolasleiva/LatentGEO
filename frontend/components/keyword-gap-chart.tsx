...
export function KeywordGapChart({ data }: KeywordGapProps) {
  ...
  const chartData = [
    {
      name: "Missing",
      value: gap_analysis.missing_keywords.length,
      fill: "#ef4444",
    },
    {
      name: "Common",
      value: gap_analysis.common_keywords.length,
      fill: "#0f766e",
    },
    {
      name: "Unique",
      value: gap_analysis.unique_keywords.length,
      fill: "#14b8a6",
    },
  ];
      ...
      <div className="glass-card p-6 rounded-2xl flex flex-col">
        ...
        <div className="mt-6 text-center p-4 glass-panel rounded-xl border border-border">
          <div className="text-4xl font-bold text-foreground tracking-tight">
            {gap_analysis.gap_score.toFixed(1)}%
          </div>
        ...
        <div className="space-y-2 flex-1 overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent">
          {gap_analysis.opportunities.slice(0, 15).map((opp, i) => (
            ...
            >
              ...
              <Badge
                variant="secondary"
                className="bg-muted text-muted-foreground border-none group-hover:bg-muted/80 group-hover:text-foreground transition-colors"
              >
                {opp.competitor_frequency}
              </Badge>
            </div>
          ))}
        </div>
