...
export default function WebhooksOpsPage() {
  ...
  const [configEvents, setConfigEvents] = useState("audit.completed\npdf.ready")
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1">
              Configurar y testear webhooks (endpoints entrantes no se invocan desde UI)
            </p>
          </div>
          <Button
            variant="outline"
            disabled={loading}
            onClick={() => run("GET /webhooks/events", async () => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card className="glass-card p-6 space-y-4">
            ...
            <div className="space-y-2">
              <Label>URL</Label>
              <Input value={configUrl} onChange={(e) => setConfigUrl(e.target.value)} placeholder="https://example.com/webhook" />
            </div>
            <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              <Button
                ...
                onClick={() => run("POST /webhooks/config", async () => {
          ...
          <Card className="glass-card p-6 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input value={testEvent} onChange={(e) => setTestEvent(e.target.value)} placeholder="audit.completed" />
            </div>
            <div className="flex flex-wrap gap-2">
              <Button
                variant="outline"
                disabled={loading || !configUrl}
                onClick={() => run("POST /webhooks/test", async () => {
              ...
              <Button
                variant="outline"
                disabled={loading}
                onClick={() => run("GET /webhooks/health", async () => {
