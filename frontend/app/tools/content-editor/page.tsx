                    ...
                    {!analysis ? (
                        ...
                        <>
                            ...
                            <div className="p-4 glass-panel rounded-xl border border-border">
                                <p className="text-sm italic text-foreground leading-relaxed">&ldquo;{analysis.summary}&rdquo;</p>
                            </div>

                            {/* Pillars */}
                            <div className="space-y-5">
                                <h3 className="font-medium text-xs uppercase tracking-widest text-muted-foreground">GEO Pillars</h3>
                                ...
                                {Object.entries(analysis.pillars).map(([key, data]: [string, any]) => (
                                    <div key={key} className="space-y-2">
                                        <div className="flex justify-between text-sm">
                                            ...
                                            <span className={data.score >= 7 ? 'text-green-500' : 'text-yellow-500'}>{data.score}/10</span>
                                        </div>
                            ...
                            <div className="space-y-4">
                                <h3 className="font-medium text-xs uppercase tracking-widest text-muted-foreground">Action Items</h3>
                                {analysis.suggestions.map((s, idx) => (
                                    <div key={idx} className="flex gap-3 items-start p-3 rounded-xl glass-panel border border-border hover:bg-muted/50 transition-colors">
                                        {s.type === 'critical' ? (
                                            <AlertCircle className="h-4 w-4 text-red-500 shrink-0 mt-0.5" />
                                        ) : (
                                            <CheckCircle className="h-4 w-4 text-green-500 shrink-0 mt-0.5" />
                                        )}
                                        <p className="text-xs leading-relaxed text-foreground">{s.text}</p>
                                    </div>
                                ))}
                            </div>
                            ...
                            {analysis.missing_entities.length > 0 && (
                                <div className="space-y-3">
                                    <h3 className="font-medium text-xs uppercase tracking-widest text-muted-foreground">Missing Entities</h3>
