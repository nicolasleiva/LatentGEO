                        ...
                        {competitorData.competitors && competitorData.competitors.length > 0 && (
                            <div className="space-y-2">
                                ...
                                {competitorData.competitors.slice(0, 5).map((comp: any, idx: number) => (
                                    <div key={idx} className="flex items-center justify-between p-3 glass-panel rounded-lg border border-border">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${idx === 0 ? 'bg-amber-500/10 text-amber-600' : 'bg-muted/60 text-muted-foreground'
                                                }`}>
                                                {idx + 1}
                                            </div>
                                            <div>
                ...
                <Card className="glass-card p-6 animate-fade-up">
                    ...
                    <div className="space-y-2">
                        {pages.slice(0, 10).map((page: any, idx: number) => (
                            <div key={idx} className="flex items-center justify-between p-4 glass-panel rounded-lg border border-border hover:bg-muted/50 transition-all">
                    ...
                    <Card className="glass-card p-6 animate-fade-up">
                        ...
                        <div className="space-y-6">
                            {Object.entries(issuesData.by_priority).map(([priority, items]: [string, any]) => (
                                items.length > 0 && (
                                    <div key={priority}>
                                        <h3 className={`text-sm font-medium mb-3 ${priority === 'CRITICAL' ? 'text-red-500' :
                                        ...
                                        <div className="space-y-2">
                                            {items.slice(0, 5).map((issue: any, idx: number) => (
                                                <div key={idx} className="p-3 glass-panel rounded-lg border border-border text-sm">
