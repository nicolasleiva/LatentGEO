...
export interface AuditsListItem {
  id: number;
  url: string;
  domain: string;
  status: string;
...
type AuditsListPageClientProps = {
  initialAudits: AuditsListItem[];
};

export default function AuditsListPageClient({
  initialAudits,
}: AuditsListPageClientProps) {
  const router = useRouter();
  const pathname = usePathname();
  const [audits, setAudits] = useState<AuditsListItem[]>(initialAudits);
          ...
          <div className="flex gap-2">
            {(
              ...
              >
                ...
                <span className="ml-1.5 text-xs text-current">
                  (
                  {status === "all"
                    ? audits.length
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h2 className="text-lg font-medium group-hover:text-brand transition-colors">
                        {audit.domain ||
                          (() => {
                            try {
