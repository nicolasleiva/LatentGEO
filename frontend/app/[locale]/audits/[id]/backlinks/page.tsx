        ...
        <div className="min-h-screen bg-background text-foreground">
            ...
            <main className="max-w-6xl mx-auto px-6 py-12 space-y-8">
                <div className="flex justify-between items-center animate-fade-up">
                    <div>
                        <h1 className="text-3xl md:text-4xl font-semibold tracking-tight">Link & Mention Analysis</h1>
                        <p className="text-muted-foreground mt-2">Internal structure, technical backlinks, and AI-powered brand analysis.</p>
                    </div>
                </div>
                ...
                <Card className="glass-card">
                    ...
                    <CardContent className="space-y-4">
                        <div className="flex flex-col md:flex-row gap-4">
                            ...
                            />
                            <Button onClick={handleAnalyze} disabled={loading} className="glass-button-primary">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <Card className="glass-card p-5">
                        <div className="text-xs uppercase tracking-widest text-muted-foreground mb-2">Total</div>
                        <div className="text-2xl font-semibold">{backlinks.length}</div>
                    </Card>
                    <Card className="glass-card p-5">
                        <div className="text-xs uppercase tracking-widest text-muted-foreground mb-2">Internal</div>
                        <div className="text-2xl font-semibold">{internalLinks.length}</div>
                    </Card>
                    <Card className="glass-card p-5">
                        <div className="text-xs uppercase tracking-widest text-muted-foreground mb-2">Technical</div>
                        <div className="text-2xl font-semibold">{technicalBacklinks.length}</div>
                    </Card>
                    <Card className="glass-card p-5">
                        <div className="text-xs uppercase tracking-widest text-muted-foreground mb-2">Mentions</div>
                                    ...
                                    <TableBody>
                                        {internalLinks.map((bl) => (
                                            <TableRow key={bl.id}>
                                                ...
                                                <TableCell>
                                                    <Badge variant="outline" className={bl.is_dofollow ? 'text-emerald-600 border-emerald-500/30' : ''}>
                                        ...
                                        {technicalBacklinks.map((bl) => (
                                            <TableRow key={bl.id}>
                                                <TableCell className="font-medium break-all">
                                                    <a href={bl.target_url} target="_blank" rel="noopener noreferrer" className="flex items-center hover:underline text-brand">
                            ...
                            <CardContent>
                                <div className="space-y-4">
                                    {brandMentions.map((bl) => {
                                        ...
                                        return (
                                            <div key={bl.id} className="border border-border rounded-lg p-4 space-y-2 glass-panel">
                                                <div className="flex items-start justify-between gap-4">
                                                    <div className="flex-1">
                                                        <a href={bl.target_url} target="_blank" rel="noopener noreferrer" className="text-brand hover:underline font-medium flex items-center">
                                                ...
                                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-muted-foreground">
                                                    <div>
                                                        <span className="font-semibold text-foreground">Topic:</span> {analysis.topic}
                                                    </div>
                                                    <div>
                                                        <span className="font-semibold text-foreground">Relevance:</span> {analysis.relevance_score}/100
                                                    </div>
                                                    <div>
                                                        <span className="font-semibold text-foreground">Follow:</span> {bl.is_dofollow ? 'Dofollow' : 'Nofollow'}
                                                    </div>
                                                </div>
                                                <div className="bg-muted/50 p-3 rounded border border-border">
                                                    <p className="text-sm"><span className="font-semibold text-foreground">Context:</span> {analysis.snippet}</p>
                                                </div>
                                                <div className="bg-brand/10 p-3 rounded border-l-4 border-brand">
                                                    <p className="text-sm"><span className="font-semibold text-foreground">Recommendation:</span> {analysis.recommendation}</p>
                                                </div>
                                                <div className="text-xs text-muted-foreground">ID: #{bl.id} · DA: {bl.domain_authority ?? '—'}</div>
                                            </div>
                                        )
                                    })}
