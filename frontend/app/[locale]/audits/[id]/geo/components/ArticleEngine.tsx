...
const TERMINAL_BATCH_STATUSES = new Set(['completed', 'failed', 'partial_failed']);
...
const normalizeArticle = (item: any): ArticleItem => ({
  ...
  keyword_strategy:
    item?.keyword_strategy && typeof item.keyword_strategy === 'object'
      ? {
          ...
          secondary_keywords: Array.isArray(item.keyword_strategy.secondary_keywords)
            ? item.keyword_strategy.secondary_keywords.map((kw: any) => safeText(kw)).filter(Boolean)
            : [],
  ...
  evidence_summary: Array.isArray(item?.evidence_summary)
    ? item.evidence_summary
        ...
        .filter((entry: EvidenceSummaryItem) => Boolean(entry.claim) || Boolean(entry.source_url))
    : [],
...
const normalizeBatch = (payload: any): BatchResponse | null => {
  ...
  return {
    ...
    summary: (payload.summary && typeof payload.summary === 'object' ? payload.summary : {}) as BatchSummary,
...
const parseErrorMessage = async (res: Response): Promise<string> => {
  ...
  try {
    ...
    if (detail && typeof detail === 'object') {
      ...
      return [code, message].filter(Boolean).join(': ') || `Request failed (${res.status})`;
    }
...
export default function ArticleEngine({ auditId, backendUrl }: ArticleEngineProps) {
  ...
  const fetchBatchStatus = useCallback(
    async (batchId: number) => {
      const res = await fetchWithBackendAuth(`${backendUrl}/api/geo/article-engine/status/${batchId}`);
      if (!res.ok) {
  ...
  useEffect(() => {
    const fetchLatest = async () => {
      ...
      try {
        const res = await fetchWithBackendAuth(`${backendUrl}/api/geo/article-engine/latest/${auditId}`);
  ...
  const generateArticles = async () => {
    ...
    setError(null);
    try {
      const res = await fetchWithBackendAuth(`${backendUrl}/api/geo/article-engine/generate`, {
        method: 'POST',
  ...
  const copyArticle = async (article: ArticleItem) => {
    const content = article.markdown?.trim() ? article.markdown : '# Article not generated yet';
    await navigator.clipboard.writeText(content);
    setCopiedIndex(article.index);
    setTimeout(() => setCopiedIndex(null), 1800);
  };

  const renderGapGroup = (label: string, items: Array<{ gap: string; impact?: string; recommended_fix?: string }> = []) => {
  ...
  return (
    <div className="space-y-6">
      <div className="bg-muted/30 border border-border rounded-xl p-6 space-y-4">
        ...
        <Button onClick={generateArticles} disabled={loading} className="glass-button-primary w-full">
          {loading ? (
      ...
      {loadingLatest && !result && (
        <div className="text-center py-10 text-muted-foreground">Loading latest generated articles...</div>
      )}
        ...
        <div className="space-y-4">
          <div className="bg-muted/30 border border-border rounded-xl p-4 grid md:grid-cols-4 gap-4">
            <div>
              ...
              <p className="text-2xl font-bold text-foreground">{result.summary.generated_count ?? 0}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Failed</p>
              <p className="text-2xl font-bold text-foreground">{result.summary.failed_count ?? 0}</p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Avg Citation Readiness</p>
              <p className="text-2xl font-bold text-foreground">
                {Number(result.summary.average_citation_readiness_score || 0).toFixed(1)}
              </p>
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Batch Status</p>
              <p className="text-2xl font-bold text-foreground capitalize">{result.status}</p>
            </div>
          </div>

          {(Array.isArray(result.articles) ? result.articles : []).map((article) => (
            <div key={`${article.index}-${article.title}`} className="bg-muted/30 border border-border rounded-xl p-6 space-y-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h3 className="text-xl font-semibold text-foreground">{article.title}</h3>
                  <p className="text-sm text-muted-foreground mt-1">
                    Keyword: <span className="text-foreground">{article.target_keyword || '-'}</span> · Focus URL: {article.focus_url || '-'}
                  </p>
                  <p className="text-sm text-muted-foreground">
                    Competitor to beat: <span className="text-foreground">{article.competitor_to_beat || 'N/A'}</span>
                  </p>
                  <p className="text-sm text-muted-foreground">
                    Status: <span className="text-foreground">{article.generation_status || 'unknown'}</span>
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-sm text-muted-foreground">Citation Score</p>
                  <p className="text-2xl font-bold text-foreground">{article.citation_readiness_score || 0}</p>
                </div>
              </div>

              {article.keyword_strategy ? (
                <div className="bg-muted/40 border border-border rounded-lg p-4 space-y-2">
                  <p className="text-sm font-medium text-foreground">Keyword Strategy</p>
                  <p className="text-sm text-muted-foreground">
                    Primary: <span className="text-foreground">{article.keyword_strategy.primary_keyword || '-'}</span>
                  </p>
                  <p className="text-sm text-muted-foreground">
                    Intent: <span className="text-foreground">{article.keyword_strategy.search_intent || '-'}</span>
                  </p>
                  {!!article.keyword_strategy.secondary_keywords?.length && (
                    <p className="text-sm text-muted-foreground">
                      Secondary: <span className="text-foreground">{article.keyword_strategy.secondary_keywords.join(', ')}</span>
                    </p>
                  )}
                </div>
              ) : null}
              ...
              {article.competitor_gap_map ? (
                <div className="bg-muted/40 border border-border rounded-lg p-4 space-y-3">
                  <p className="text-sm font-medium text-foreground">Competitor Gaps</p>
                  {renderGapGroup('Content', article.competitor_gap_map.content)}
                  {renderGapGroup('Schema', article.competitor_gap_map.schema)}
                  {renderGapGroup('E-E-A-T', article.competitor_gap_map.eeat)}
                  {renderGapGroup('Clarity', article.competitor_gap_map.clarity)}
                  {renderGapGroup('FAQ', article.competitor_gap_map.faq)}
                  {renderGapGroup('Evidence', article.competitor_gap_map.evidence)}
                </div>
              ) : null}

              {article.generation_error ? (
                <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-4 text-sm text-red-300">
                  <strong>{article.generation_error.code || 'Generation error'}:</strong> {article.generation_error.message || 'Unknown failure'}
                </div>
              ) : null}

              <div className="flex gap-2">
                <Button variant="outline" onClick={() => copyArticle(article)} className="border-border/70 text-foreground">
              ...
              <Textarea value={article.markdown || ''} readOnly className="bg-muted/50 border-border text-foreground min-h-[240px]" />

              {!!article.evidence_summary?.length && (
                <div>
                  <p className="text-sm text-muted-foreground mb-1">Evidence Summary</p>
                  <div className="space-y-1">
                    {article.evidence_summary.slice(0, 5).map((evidence, idx) => (
                      <p key={`ev-${idx}`} className="text-sm text-muted-foreground">
                        • {evidence.claim} {evidence.source_url ? `(${evidence.source_url})` : ''}
                      </p>
                    ))}
                  </div>
                </div>
