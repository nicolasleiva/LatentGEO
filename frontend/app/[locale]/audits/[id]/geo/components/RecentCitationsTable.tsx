...
export default function RecentCitationsTable({
  auditId,
  backendUrl,
}: RecentCitationsTableProps) {
    ...
    return (
      <div className="text-center py-12 bg-muted/30 rounded-2xl border border-dashed border-border">
        ...
        <p className="text-muted-foreground">
          No citations found yet. Start tracking to see results.
        </p>
      </div>
    );
  }
      ...
      {citations.map((citation) => (
        ...
        >
          ...
          <p className="text-muted-foreground text-sm mb-3 line-clamp-2">
            {citation.response_preview}
          </p>
          <div className="flex items-center gap-4 text-xs text-muted-foreground">
            <span
              className={`px-2 py-1 rounded ${
                citation.citation_type === "direct"
                  ? "bg-green-500/20 text-green-400"
                  : citation.citation_type === "indirect"
