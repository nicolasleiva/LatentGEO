    ...
    return (
      <div className="text-center py-12 bg-muted/30 rounded-2xl border border-dashed border-border">
        ...
        <p className="text-muted-foreground">
          No historical data available yet. Data will appear after tracking for
          multiple months.
        </p>
      </div>
    );
    ...
    <div className="space-y-6">
      {history.map((month, idx) => {
        ...
        const trend = prevMonth
          ? month.citation_rate - prevMonth.citation_rate
          : 0;
            ...
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="text-muted-foreground text-sm">
                  {month.queries_tracked} queries tracked
                </p>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-2xl font-bold text-foreground">
                  {month.citation_rate.toFixed(1)}%
                </span>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {month.top_queries.map((query, qidx) => (
                    <span
                      key={qidx}
                      className="bg-muted/40 text-muted-foreground px-3 py-1 rounded-lg text-sm"
                    >
                      {query}
                    </span>
                  ))}
