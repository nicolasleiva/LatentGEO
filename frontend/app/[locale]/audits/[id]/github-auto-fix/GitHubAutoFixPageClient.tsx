...
type GitHubAutoFixPageClientProps = {
  auditId?: string;
  locale?: string;
  initialAudit?: any | null;
  initialMissingInputs?: FixInputGroup[];
  hasInitialMissingInputs?: boolean;
  initialConnections?: any[];
  hasInitialConnections?: boolean;
  initialRepositories?: any[];
};

const getConnectionLabel = (connection: any) =>
  connection?.github_username || connection?.username || "GitHub";

export default function GitHubAutoFixPageClient({
  auditId: auditIdProp,
  locale: localeProp,
  initialAudit = null,
  initialMissingInputs = [],
  hasInitialMissingInputs = false,
  initialConnections = [],
  hasInitialConnections = false,
  initialRepositories = [],
}: GitHubAutoFixPageClientProps) {
  ...
  const [repositories, setRepositories] = useState<any[]>(initialRepositories);
  const [selectedConnection, setSelectedConnection] = useState<string | null>(
    initialConnections[0]?.id ? String(initialConnections[0].id) : null,
  );
  const [selectedRepo, setSelectedRepo] = useState<string | null>(
    initialRepositories[0]?.id ? String(initialRepositories[0].id) : null,
  );
  const [loading, setLoading] = useState(!initialAudit);
  const [reposLoading, setReposLoading] = useState(false);
  const [creating, setCreating] = useState(false);
  const [prResult, setPrResult] = useState<any>(null);
  const [missingInputs, setMissingInputs] =
    useState<FixInputGroup[]>(initialMissingInputs);
  const [missingInputsLoading, setMissingInputsLoading] = useState(false);
  const [guidedInputsEnabled, setGuidedInputsEnabled] = useState(
    hasInitialMissingInputs,
  );
  ...
  useEffect(() => {
    ...
    if (hasInitialMissingInputs) {
      setMissingInputsLoading(false);
    }
    if (!hasInitialConnections) {
  ...
  const loadGuidedInputs = async () => {
    setGuidedInputsEnabled(true);
    if (hasInitialMissingInputs || missingInputs.length > 0) {
      return;
    }
    await fetchMissingInputs();
  };
  ...
  const fetchConnections = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setConnections(data);
        if (data.length > 0) {
          const connectionId = String(data[0].id);
  ...
  useEffect(() => {
    if (
      ...
      void fetchRepositories(selectedConnection);
    }
  }, [
    fetchRepositories,
    hasInitialConnections,
  ...
  const connectGitHub = async () => {
    try {
      const returnTo = `/${locale}/audits/${auditId}/github-auto-fix`;
      const res = await fetchWithBackendAuth(
        `${backendUrl}/api/v1/github/auth-url?return_to=${encodeURIComponent(returnTo)}`,
      );
      const data = await safeParseJson(res);
      if (!res.ok || !data?.url) {
    ...
    <div className="flex min-h-screen flex-col pb-20 bg-background text-foreground">
      ...
      <main className="flex-1 container mx-auto px-6 py-8">
        ...
        {loading && connections.length === 0 && (
          <Card className="glass-card border border-border p-12">
          ...
          <div className="space-y-6">
            ...
            <Card className="glass-card p-8 border border-border">
              ...
              <div className="space-y-6">
                ...
                {!guidedInputsEnabled && missingInputs.length === 0 ? (
                  <div className="glass-panel border border-border rounded-xl p-6 space-y-4">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        Load the assistant only if the delivery flow needs extra
                        content inputs. This keeps the initial route lighter.
                      </p>
                    </div>
                    <Button
                      variant="outline"
                      onClick={() => void loadGuidedInputs()}
                    >
                      Load Guided Assistant
                    </Button>
                  </div>
                ) : (
                  <div
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        Kimi proposes audit-grounded values. Required fields
                        must be saved before PR creation.
                      </p>
                    </div>
                    ...
                    {missingInputsLoading ? (
                      ...
                      <div className="text-sm text-emerald-600">
                        All required inputs are complete. You can generate the
                        PR.
                      </div>
                    ) : (
                      <div className="space-y-6">
