...
describe('GEO tool menu', () => {
  beforeEach(() => {
    ...
    ;(fetchWithBackendAuth as jest.Mock).mockResolvedValue({
      ...
      json: async () => ({
        audit_id: 3,
        citation_tracking: { citation_rate: 0, total_queries: 0, mentions: 0, sentiment_breakdown: {} },
        top_opportunities: [],
  ...
  it('renders grouped menu and switches tab through selector', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Tool menu')).toBeInTheDocument())

    const trigger = screen.getByRole('combobox')
    await user.click(trigger)
    await user.click(screen.getByText('Competitors'))

    expect(mockReplace).toHaveBeenCalledWith('/en/audits/3/geo?tab=competitors', { scroll: false })
  })
})
