...
describe('HomePage', () => {
  ...
  it('renders hero copy and core actions for growth positioning', () => {
    ...
    expect(screen.getByText(/turn product pages into ai-citable sources/i)).toBeInTheDocument()
    expect(screen.getByText(/from url -> prioritized fixes -> github prs \+ tests/i)).toBeInTheDocument()
    expect(screen.getByText(/built for chatgpt, perplexity & generative search/i)).toBeInTheDocument()

    expect(screen.getByPlaceholderText(/paste your website url/i)).toBeInTheDocument()
    expect(screen.getByRole('button', { name: /start free audit/i })).toBeInTheDocument()
  })

  it('renders proof chips and avoids legacy keyword-heavy headline copy', () => {
    render(<HomePage />)

    expect(screen.getByText(/rank products in ai answers/i)).toBeInTheDocument()
    expect(screen.getByText(/get cited as a trusted source/i)).toBeInTheDocument()
    expect(screen.getByText(/turn citations into qualified clicks and sales/i)).toBeInTheDocument()
  ...
  it('shows validation feedback for an invalid URL', async () => {
    ...
    await user.type(screen.getByPlaceholderText(/paste your website url/i), 'not-a-url')
    await user.click(screen.getByRole('button', { name: /start free audit/i }))

    expect(await screen.findByText(/please enter a valid domain or url/i)).toBeInTheDocument()
  })
  ...
  it('accepts a domain without protocol and normalizes it to https', async () => {
    ...
    await user.type(screen.getByPlaceholderText(/paste your website url/i), 'ceibo.digital')
    await user.click(screen.getByRole('button', { name: /start free audit/i }))
  ...
  it('loads recent audits for authenticated users', async () => {
    ...
    await waitFor(() => {
      expect(fetchWithBackendAuth).toHaveBeenCalledWith('http://localhost:8000/api/audits')
    })
