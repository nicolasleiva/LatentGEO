...
describe('Audit detail GEO tools navigation', () => {
        ...
        return Promise.resolve({
          ...
          json: async () => ({
            ...
            target_audit: {
              ...
              structure: { h1_check: { status: 'warn' }, semantic_html: { score_percent: 17 } },
  ...
  it('renders GEO tool links with locale path and prefetches critical routes', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('heading', { name: /seo & geo tools/i })).toBeInTheDocument()
    })

    const geoDashboard = screen.getByRole('link', { name: /geo dashboard/i })
    const githubAutoFix = screen.getByRole('link', { name: /github auto-fix/i })
    const articleEngine = screen.getByRole('link', { name: /article engine/i })

    expect(geoDashboard).toHaveAttribute('href', '/en/audits/6/geo')
    expect(githubAutoFix).toHaveAttribute('href', '/en/audits/6/github-auto-fix')
    expect(articleEngine).toHaveAttribute('href', '/en/audits/6/geo?tab=article-engine')
