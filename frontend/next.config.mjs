...
const rawApiUrl = process.env.NEXT_PUBLIC_API_URL || process.env.API_URL || process.env.NEXT_PUBLIC_BACKEND_URL || ''
const apiOrigin = (() => {
  try {
...
const auth0Origin = process.env.AUTH0_DOMAIN ? `https://${process.env.AUTH0_DOMAIN}` : ''

if (isProd && !apiOrigin) {
  throw new Error('NEXT_PUBLIC_API_URL or API_URL is required to build for production')
}
