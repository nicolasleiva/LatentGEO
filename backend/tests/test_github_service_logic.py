...
async def test_create_pr_with_fixes_resets_branch():
    ...
    with patch(
        "app.integrations.github.service.GitHubOAuth.decrypt_token",
        return_value="decrypted_token",
    ), patch(
        "app.integrations.github.service.GitHubClient", return_value=mock_client
    ), patch(
        "app.integrations.github.service.PRGeneratorService"
    ) as mock_pr_gen, patch(
        "app.integrations.github.service.CodeModifierService"
    ) as mock_code_mod:
