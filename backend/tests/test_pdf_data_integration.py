...
from app.services.rank_tracking_service import RankTrackingService as RankTrackingServicePlural
...
async def test_pdf_service_real_data_flow():
    ...
    with patch("app.services.keyword_service.KeywordService.research_keywords", new_callable=AsyncMock) as mock_research:
        mock_research.return_value = []
        with patch("app.services.backlink_service.BacklinkService.analyze_backlinks", new_callable=AsyncMock) as mock_backlinks:
            ...
            with patch("app.core.llm_kimi.get_llm_function", return_value=AsyncMock()):
                with patch("app.services.pipeline_service.PipelineService.generate_report", new_callable=AsyncMock) as mock_gen:
                    ...
                    with patch("app.services.pdf_service.PDFService.generate_comprehensive_pdf", new_callable=AsyncMock) as mock_pdf_gen:
                        ...
                        with patch("app.services.rank_tracker_service.RankTrackerService.track_rankings", new_callable=AsyncMock) as mock_rank:
                            mock_rank.return_value = []
                            with patch("app.services.llm_visibility_service.LLMVisibilityService.generate_llm_visibility", new_callable=AsyncMock) as mock_vis:
                                mock_vis.return_value = []
                                with patch("app.services.ai_content_service.AIContentService.generate_content_suggestions", return_value=[]) as mock_ai:
                                    ...
                                    assert passed_keywords.get('total_keywords') == 1, "Keywords should be present even if Backlinks failed"
                                    
                                    # Verify Backlinks were passed as empty dict
                                    passed_backlinks = kwargs.get('backlinks_data', {})
                                    assert passed_backlinks == {}, "Backlinks should be empty dict on failure"
...
async def test_pdf_service_fallback_to_db():
    ...
    with patch("app.services.keyword_service.KeywordService.research_keywords", side_effect=Exception("API Fail")) as mock_kw_svc:
         with patch("app.services.backlink_service.BacklinkService.analyze_backlinks", side_effect=Exception("API Fail")) as mock_bl_svc:
             with patch("app.services.pagespeed_service.PageSpeedService.analyze_both_strategies", side_effect=Exception("API Fail")) as mock_ps_svc:
                 with patch("app.services.rank_tracker_service.RankTrackerService.track_rankings", new_callable=AsyncMock) as mock_rank:
                     mock_rank.return_value = [] # Let rank succeed or fail, doesn't matter for this test
                    ...
                    with patch("app.services.pipeline_service.PipelineService.generate_report", new_callable=AsyncMock) as mock_gen:
                        ...
                        with patch("app.services.pdf_service.PDFService.generate_comprehensive_pdf", new_callable=AsyncMock) as mock_pdf_gen:
                            ...
                            with patch("app.services.llm_visibility_service.LLMVisibilityService.generate_llm_visibility", new_callable=AsyncMock):
                                with patch("app.services.ai_content_service.AIContentService.generate_content_suggestions", return_value=[]):
