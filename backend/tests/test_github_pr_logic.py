...
def test_map_issue_to_fix_type(mock_db):
    ...
    # Test valid mappings
    assert (
        service._map_issue_to_fix_type("Missing meta description") == "meta_description"
    )
    ...
    assert (
        service._map_issue_to_fix_type("Keyword density low") == "content_improvement"
    )
    ...
    assert (
        service._map_issue_to_fix_type("", "PRODUCT_CONTENT_GAP")
        == "content_enhancement"
    )
...
def test_prepare_fixes_from_audit_fallback(mock_db):
    ...
    assert (
        fixes[0]["description"] == "Optimize page title for SEO and Click-Through Rate"
    )
