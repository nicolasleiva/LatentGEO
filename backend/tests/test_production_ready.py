...
def test_app(test_db):
    ...
    with patch.object(settings, "DATABASE_URL", db_url), patch.object(
        settings, "DEBUG", False
    ), patch.object(database, "engine", engine), patch.object(
...
class TestInputValidation:
    def test_ssrf_blocking(self, client):
        ...
        response = client.post(
            "/api/v1/audits/", json={"url": "http://localhost:8080/admin"}
        )
...
class TestAuthentication:
    def test_jwt_flow(self):
        ...
        with patch.dict(
            os.environ, {"SECRET_KEY": "a-very-secret-test-key-that-is-long-enough"}
        ):
