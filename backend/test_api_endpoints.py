...
def test_rank_tracking():
    ...
    params = {"domain": "v0-auditor-geo-landing-page.vercel.app"}
    try:
        response = requests.post(url, params=params, json=["seo auditor", "geo seo tool"], timeout=90)
        print(f"Status Code: {response.status_code}")
        if response.status_code == 200:
...
def test_llm_visibility():
    ...
    try:
        response = requests.post(url, params=params, json=["Is Auditor GEO a good tool?"], timeout=90)
        print(f"Status Code: {response.status_code}")
        if response.status_code == 200:
