...
def test_health_endpoint():
    ...
    try:
        ...
        assert (
            data.get("services", {}).get("database") == "connected"
        ...
        pytest.skip(
            "Servidor no está corriendo (localhost:8000) — saltando health test"
        )
...
def test_rate_limiting():
    ...
    try:
        ...
        if not responses:
            ...
            pytest.skip(
                "Servidor no disponible en localhost:8000 — saltando test de rate limiting"
            )

        assert (
            429 in responses or len(responses) > 0
        ), "Rate limiting no activado y respuestas vacías"
