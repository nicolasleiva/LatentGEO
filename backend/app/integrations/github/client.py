...
class GitHubClient:
        ...
        try:
            ...
            if "package.json" in file_names:
                pkg_content = base64.b64decode(
                    file_names["package.json"].content
                ).decode("utf-8")
            ...
            if config["site_type"] == "unknown":
                if any(
                    f in file_names
                    for f in ["config.toml", "config.yaml", "config.yml"]
                ):
    ...
    def create_branch(
        self, repo: Repository.Repository, branch_name: str, base_branch: str = None
    ) -> str:
        """
        ...
        try:
            ...
            logger.warning(
                f"Branch {branch_name} already exists in {repo.full_name}, deleting it"
            )
            existing_ref.delete()
    ...
    def _scan_for_nextjs_pages(
        self, repo: Repository.Repository, base_path: str
    ) -> List[str]:
    ...
    def _scan_for_html_files(
        ...
        try:
            contents = (
                repo.get_contents(base_path) if base_path else repo.get_contents("")
            )
            for content in contents:
    ...
    def get_file_content(
        self, repo: Repository.Repository, file_path: str, ref: str = None
    ) -> str:
        """
