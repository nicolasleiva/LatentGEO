...
class GEOBlogAuditor(BlogAuditorService):
    ...
    def _calculate_geo_summary(self, blogs: List[Dict]) -> Dict[str, Any]:
        ...
        category_counts = {"structure": 0, "eeat": 0, "content": 0}
        ...
        for blog in blogs:
            ...
            citation_distribution[potential] = (
                citation_distribution.get(potential, 0) + 1
            )
        ...
        return {
            ...
            "blogs_ready_for_llms": citation_distribution.get("very_high", 0)
            + citation_distribution.get("high", 0),
            "blogs_need_improvement": total
            - (
                citation_distribution.get("very_high", 0)
                + citation_distribution.get("high", 0)
            ),
            ...
            "priority_actions": self._get_priority_actions(
                category_counts, avg_geo_score
            ),
        }

    def _get_priority_actions(
        self, category_counts: Dict, avg_score: float
    ) -> List[str]:
        ...
        if avg_score < 60:
            actions.insert(
                0, "🚨 CRÍTICO: Score GEO general bajo. Revisar manual de GEO completo"
            )

        return actions

    def generate_geo_fixes_from_audit(
