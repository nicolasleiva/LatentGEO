...
class PRGeneratorService:
    ...
    def generate_pr_body(
        audit_data: Dict, fixes: List[Dict], file_changes: Dict
    ) -> str:
        """
        ...
        body += (
            "- **Dates**: Verify publication and modification dates are accurate\n\n"
        )
        ...
        body += (
            f"- **Medium Priority Issues**: {audit_data.get('medium_issues', 0)}\n\n"
        )
    ...
    def _generate_impact_section(audit_data: Dict, fixes: List[Dict]) -> str:
        ...
        # GEO Impact
        if any(
            fix.get("type") in ["conversational_tone", "schema", "eeat"]
            for fix in fixes
        ):
            ...
            impact += (
                "- **Answer Engineering**: Optimized for AI-powered search results\n\n"
            )
        ...
        if any(fix.get("type") == "alt_text" for fix in fixes):
            ...
            impact += (
                "- **Screen Readers**: Added alt text helps visually impaired users\n"
            )
            impact += (
                "- **Image SEO**: Search engines can now index your images properly\n\n"
            )
    ...
    def calculate_expected_score_improvement(
        audit_data: Dict, fixes: List[Dict]
    ) -> Dict[str, float]:
        """
