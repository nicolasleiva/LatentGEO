...
class CodeModifierService:
    ...
    def _apply_fixes_to_html(html_content: str, fixes: List[Dict]) -> str:
        ...
        for fix in fixes:
            ...
            if fix_type == "meta_description":
                ...
                if CodeModifierService._update_meta_property(soup, "og:description", value):
                    modified = True
            
            elif fix_type == "alt_text":
                # value debe contener {"selector": "img[src='...']", "alt": "..."}
                if CodeModifierService._update_image_alt(soup, fix.get("selector"), value):
                    modified = True
        ...
        for fix in fixes:
            ...
            if site_type == "gatsby":
                ...
                if "import { Helmet }" in content or "import Helmet" in content:
                    ...
                    modified_content = CodeModifierService._update_gatsby_head(modified_content, fix_type, value)
    ...
    @staticmethod
    def _update_meta_property(soup: BeautifulSoup, property_name: str, content: str) -> bool:
    ...
    @staticmethod
    def _update_image_alt(soup: BeautifulSoup, selector: Optional[str], alt_text: str) -> bool:
        ...
        if match:
            ...
            if fix_type == "title":
                ...
                if re.search(title_pattern, metadata_block):
                    updated_block = re.sub(title_pattern, f'title: "{value}"', metadata_block)
                else:
                    # Agregar title
                ...
                if re.search(desc_pattern, metadata_block):
                    updated_block = re.sub(desc_pattern, f'description: "{value}"', metadata_block)
                else:
        ...
        if match:
            ...
            if fix_type == "title":
                ...
                if re.search(title_pattern, head_content):
                    updated_head = re.sub(title_pattern, f'<title>{value}</title>', head_content)
                else:
