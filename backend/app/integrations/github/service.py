...
class GitHubService:
    ...
    def _map_issue_to_fix_type(self, issue: str, issue_code: str = "") -> str:
        ...
        if any(
            term in issue_lower
            for term in ["meta description", "description tag", "meta desc"]
        ):
        ...
        if any(
            term in issue_lower
            for term in ["alt text", "alt attribute", "image alt", "missing alt"]
        ):
            return "alt_text"
        ...
        if any(
            term in issue_lower for term in ["schema", "structured data", "json-ld"]
        ):
            return "schema"
        ...
        if any(
            term in issue_lower
            for term in ["content", "text", "readability", "keyword", "topic"]
        ):
    ...
    async def generate_fixes_with_context(
        self, audit_id: int, repo_owner: str, repo_name: str
    ) -> List[Dict[str, Any]]:
        """
        ...
        if context.get("keywords"):
            missing_keywords = [
                k for k in context["keywords"] if k.get("search_volume", 0) > 1000
            ]
            if missing_keywords:
                fixes.append(
                    {
        ...
        if context.get("llm_visibility"):
            ...
            mentions = len([item for item in llm_vis if item.get("mentioned")])
            if mentions < 3:
                fixes.append(
                    {
                        "type": "geo",
        ...
        if context.get("llm_visibility"):
            invisible_queries = [
                item
                for item in context["llm_visibility"]
                if not item.get("is_visible", False)
            ]
        ...
        logger.info(
            f"Generated {len(fixes)} fixes for audit {audit_id} using complete context"
        )
        return fixes
    ...
    async def create_or_update_connection(
        ...
        connection = (
            ...
            .filter(GitHubConnection.github_user_id == github_user_id)
            .first()
        )
    ...
    async def get_valid_client(self, connection_id: str) -> GitHubClient:
        ...
        connection = (
            self.db.query(GitHubConnection)
            .filter(GitHubConnection.id == connection_id)
            .first()
        )
    ...
    async def sync_repositories(self, connection_id: str) -> List[GitHubRepository]:
        ...
        for gr in github_repos:
            ...
            if not repo:
                repo = GitHubRepository(
                    connection_id=connection_id, github_repo_id=gr["github_repo_id"]
                )
                self.db.add(repo)
        ...
        self.db.commit()
        logger.info(
            f"Synced {len(synced_repos)} repositories for connection {connection_id}"
        )
        return synced_repos

    async def analyze_repository(
        self, connection_id: str, repo_id: str
    ) -> GitHubRepository:
        """
    ...
    async def create_pr_with_fixes(
        self, connection_id: str, repo_id: str, audit_id: int, fixes: List[Dict]
    ) -> GitHubPullRequest:
        """
        ...
        if audit:
            try:
                ...
                if audit.keywords:
                    audit_context["keywords"] = [
                        k.term for k in audit.keywords[:20]
                    ]  # Top 20 keywords
                ...
                if not audit_context["issues"]:
                    if audit.critical_issues > 0:
                        audit_context["issues"].append(
                            f"{audit.critical_issues} Critical Issues detected"
                        )
                    if audit.high_issues > 0:
                        audit_context["issues"].append(
                            f"{audit.high_issues} High Priority Issues detected"
                        )
                ...
                if audit.pagespeed_data:
                    ...
                    audit_context["pagespeed"] = {
                        "mobile": {
                            ...
                            "metrics": {
                                "LCP": mobile.get("metrics", {}).get("lcp")
                                or mobile.get("core_web_vitals", {}).get("lcp"),
                                "FCP": mobile.get("metrics", {}).get("fcp")
                                or mobile.get("core_web_vitals", {}).get("fcp"),
                                "CLS": mobile.get("metrics", {}).get("cls")
                                or mobile.get("core_web_vitals", {}).get("cls"),
                        ...
                        "desktop": {
                            ...
                            "metrics": {
                                "LCP": desktop.get("metrics", {}).get("lcp")
                                or desktop.get("core_web_vitals", {}).get("lcp"),
                                "FCP": desktop.get("metrics", {}).get("fcp")
                                or desktop.get("core_web_vitals", {}).get("fcp"),
                                "CLS": desktop.get("metrics", {}).get("cls")
                                or desktop.get("core_web_vitals", {}).get("cls"),
                    ...
                    if opps:
                        for key, value in opps.items():
                            if (
                                ...
                                and value.get("numericValue")
                                and value.get("numericValue") > 0
                            ):
                        ...
                        # Sort by savings
                        audit_context["pagespeed"]["opportunities"].sort(
                            key=lambda x: x["savings_ms"], reverse=True
                        )
                        audit_context["pagespeed"]["opportunities"] = audit_context[
                            "pagespeed"
                        ]["opportunities"][:5]
                    ...
                    if diags:
                        ...
                        audit_context["pagespeed"]["diagnostics"] = audit_context[
                            "pagespeed"
                        ]["diagnostics"][:5]
                ...
                if audit.target_audit:
                    ...
                    audit_context["technical_audit"] = {
                        "schema_status": ta.get("schema", {})
                        .get("schema_presence", {})
                        .get("status"),
                        "h1_status": ta.get("structure", {})
                        .get("h1_check", {})
                        .get("status"),
                        "meta_description": ta.get("meta", {})
                        .get("meta_description", {})
                        .get("exists"),
                        "canonical": ta.get("meta", {})
                        .get("canonical", {})
                        .get("exists"),
                        "semantic_html_score": ta.get("structure", {})
                        .get("semantic_html", {})
                        .get("score_percent"),
                    }
                    ...
                    audit_context["content_suggestions"] = [
                        {
                            ...
                            "suggestion": s.content_outline,
                        }
                        for s in audit.ai_content_suggestions[:5]
                    ]

                logger.info(
                    f"Extracted rich audit context: {len(audit_context['keywords'])} keywords, PageSpeed: {'Yes' if audit_context['pagespeed'] else 'No'}"
                )
        ...
        for file_path in page_files[:10]:  # Limitar a 10 archivos para el MVP
            try:
                ...
                original_content = client.get_file_content(
                    gh_repo, file_path, repo.default_branch
                )

                # Aplicar fixes
                modified_content = CodeModifierService.apply_fixes(
                    original_content, file_path, fixes, repo.site_type, audit_context
                )

                # Si hubo cambios, actualizar archivo
                if modified_content != original_content:
                    commit_message = PRGeneratorService.generate_commit_message(
                        file_path, fixes[0].get("type") if fixes else "seo"
                    )
        ...
        pr_data = client.create_pull_request(
            gh_repo, pr_title, pr_body, branch_name, repo.default_branch
        )
        ...
        pr = GitHubPullRequest(
            ...
            expected_improvements=PRGeneratorService.calculate_expected_score_improvement(
                audit_data, fixes
            ),
        )
    ...
    async def get_repository_prs(self, repo_id: str) -> List[GitHubPullRequest]:
        ...
        return (
            self.db.query(GitHubPullRequest)
            .filter(GitHubPullRequest.repository_id == repo_id)
    ...
    async def update_pr_status(self, pr_id: str, status: PRStatus) -> GitHubPullRequest:
        ...
        pr = (
            self.db.query(GitHubPullRequest)
            .filter(GitHubPullRequest.id == pr_id)
            .first()
        )
