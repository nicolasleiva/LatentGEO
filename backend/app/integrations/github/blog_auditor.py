...
class BlogAuditorService:
    ...
    async def audit_all_blogs(self, repo_full_name: str, site_type: str) -> Dict[str, Any]:
        """
        Audita todos los blogs de un repositorio
        ...
        try:
            if site_type == "nextjs":
                ...
                try:
                    ...
                    blog_files.extend(self._scan_directory_for_pattern(
                ...
                try:
                    blog_files.extend(self._scan_directory_for_extension(
                ...
                try:
                    ...
                    for content in root_contents:
                        ...
                        if content.type == "file" and content.name.endswith(('.tsx', '.jsx')):
                            # Excluir archivos de configuración comunes
                            if content.name not in ['vite.config.tsx', 'vite.config.jsx']:
    ...
    def _scan_directory_for_pattern(self, repo, path: str, patterns: List[str]) -> List[str]:
        ...
        try:
            ...
            for content in contents:
                if content.type == "dir":
                    files.extend(self._scan_directory_for_pattern(repo, content.path, patterns))
    ...
    def _scan_directory_for_extension(self, repo, path: str, extensions: List[str]) -> List[str]:
        ...
        try:
            ...
            for content in contents:
                if content.type == "dir":
                    files.extend(self._scan_directory_for_extension(repo, content.path, extensions))
    ...
    async def _audit_single_blog(self, repo, file_path: str, site_type: str) -> Dict[str, Any]:
        """
        ...
        if blog_data.get("published_date"):
            try:
                pub_date = datetime.fromisoformat(blog_data["published_date"].replace('Z', '+00:00'))
    ...
    def _parse_blog_content(self, content: str, file_path: str, site_type: str) -> Dict[str, Any]:
        """
        ...
        if file_path.endswith(('.md', '.mdx', '.markdown')):
            try:
                ...
                data["images_without_alt"] = len([img for img in img_matches if not img[0]])
        ...
        # React/Next.js/Astro components
            ...
            # Schema check
            data["has_schema"] = 'application/ld+json' in content or 'schema.org' in content
    ...
    def _extract_slug_from_path(self, file_path: str) -> str:
        ...
        # Markdown files
        filename = parts[-1].replace('.md', '').replace('.mdx', '').replace('.markdown', '')
    ...
    def generate_fixes_from_audit(self, blog_audit: Dict[str, Any]) -> List[Dict[str, Any]]:
        """
