...
class HubSpotConnection(Base):
    ...
    pages = relationship("HubSpotPage", back_populates="connection", cascade="all, delete-orphan")
...
class HubSpotPage(Base):
    ...
    id = Column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    connection_id = Column(String(36), ForeignKey("hubspot_connections.id"), nullable=False)
    ...
    changes = relationship("HubSpotChange", back_populates="page", cascade="all, delete-orphan")
