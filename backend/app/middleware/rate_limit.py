...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app):
        ...
        try:
            ...
            logger.warning(f"Rate Limiter: Redis unavailable ({e}). Falling back to in-memory strategy.")
    ...
    def _check_redis_limit(self, key: str, max_requests: int, window: int) -> Tuple[bool, int, int]:
    ...
    def _check_memory_limit(self, key: str, max_requests: int, window: int) -> Tuple[bool, int, int]:
        ...
        if now - self.last_cleanup > self.cleanup_interval:
            for k in list(self.memory_requests.keys()):
                self.memory_requests[k] = [ts for ts in self.memory_requests[k] if now - ts < window]
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if self.use_redis:
            allowed, count, reset_after = self._check_redis_limit(client_key, max_requests, window)
        else:
            allowed, count, reset_after = self._check_memory_limit(client_key, max_requests, window)
