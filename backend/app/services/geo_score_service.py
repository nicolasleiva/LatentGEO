...
class GEOScoreService:
    ...
    async def calculate_site_geo_score(self, url: str, audit_id: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        citation_score = await self._calculate_citation_score(url)
    ...
    async def _calculate_schema_score(self, audit_data: Optional[Dict]) -> float:
        ...
        schema_issues = [
            fix for fix in fix_plan 
            if "schema" in fix.get("issue", "").lower() or 
               "structured data" in fix.get("issue", "").lower()
        ]
    ...
    async def _calculate_citation_score(self, url: str) -> float:
        """
        Score de citaciones actuales (0-100)
        
        Evalúa cuántas veces el sitio es citado en LLMs
        """
        # TODO: Implement citation tracking when service is ready
        # For now, return a neutral score
        logger.debug(f"Citation tracking not yet implemented, returning neutral score")
        return 50.0  # Neutral score until citation tracker is properly initialized
    ...
    async def compare_with_competitors(self, url: str, competitor_urls: List[str]) -> Dict[str, Any]:
        """
        ...
        if competitor_scores:
            avg_competitor_score = sum(c["score"] for c in competitor_scores) / len(competitor_scores)
