...
class KeywordService:
    ...
    async def _research_kimi(
        ...
        try:
            ...
            logger.info(f"Keyword research prompt prepared for {domain} in {(prompt_time - start_time)*1000:.1f}ms")
            ...
            logger.info(f"Kimi API call for {domain} completed in {api_duration_ms:.1f}ms")
            ...
            logger.info(f"Kimi generó {len(results)} keywords para {domain} en {total_duration_ms:.1f}ms (API: {api_duration_ms:.1f}ms)")
            return results
