...
class CitationTrackerService:
    ...
    async def track_citations(
        ...
        citation_rate = (
            (mentioned_count / total_queries * 100) if total_queries > 0 else 0
        )

        logger.info(
            f"Citation Tracking completado: {mentioned_count}/{total_queries} ({citation_rate:.1f}%)"
        )
    ...
    def _generate_tracking_queries(
        brand_name: str, industry: str, keywords: List[str] = None
    ) -> List[str]:
    ...
    def _extract_citation_context(text: str, brand_name: str, domain: str) -> str:
        ...
        if pos == -1:
            # Buscar por dominio
            domain_clean = (
                domain.replace("www.", "").replace(".com", "").replace(".io", "")
            )
    ...
    def get_citation_history(
        db: Session, audit_id: int, days: int = 30
    ) -> Dict[str, Any]:
        """
