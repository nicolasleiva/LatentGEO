...
class QueryDiscoveryService:
    ...
    async def discover_queries(
        ...
        classified_queries = QueryDiscoveryService._classify_queries(validated_queries)

        # PASO 4: Rankear por potencial
        ranked_queries = QueryDiscoveryService._rank_by_potential(classified_queries)

        logger.info(
            f"Query discovery completado: {len(ranked_queries)} queries encontradas"
        )
...
"""
    ...
    async def _validate_queries(
        ...
        for query in queries:
            try:
                ...
                mentions_brand = (
                    brand_name.lower() in response.lower()
                    or domain.replace("www.", "").replace(".com", "")
                    in response.lower()
                )
    ...
    @staticmethod
    def save_discovered_queries(db: Session, audit_id: int, queries: List[Dict]):
    ...
    def get_top_opportunities(
        db: Session, audit_id: int, limit: int = 10
    ) -> List[Dict]:
        """
            ...
            opportunities = (
                ...
                .filter(
                    and_(
                        ...
                        not DiscoveredQuery.mentions_brand,  # No menciona aún
