...
class ContentTemplateService:
    ...
    async def generate_template(
        ...
        personalized_structure = ContentTemplateService._personalize_structure(
            template["structure"], topic, keywords
        )
        ...
        if llm_function:
            example_content = await ContentTemplateService._generate_example_content(
                template_type, topic, keywords, llm_function
            )
    ...
    def _personalize_structure(
        structure: List[str], topic: str, keywords: List[str]
    ) -> List[str]:
    ...
    async def _generate_example_content(
        template_type: str, topic: str, keywords: List[str], llm_function: callable
    ) -> str:
...
- [Requisito 3]""",
    ...
    def analyze_content_for_geo(content: str) -> Dict[str, Any]:
        ...
        has_lists = (
            "- " in content
            or "* " in content
            or "<ul" in content_lower
            or "<ol" in content_lower
        )
        ...
        if has_headings:
            ...
            analysis["improvements"].append(
                "Agregar headings (H2, H3) para mejor estructura"
            )

        if has_lists:
            analysis["strengths"].append(
                "Incluye listas (fáciles de procesar para LLMs)"
            )
            analysis["score"] += 15
        else:
            analysis["improvements"].append(
                "Convertir parágrafos largos en listas cuando sea posible"
            )
        ...
        if len(long_paragraphs) > 3:
            analysis["improvements"].append(
                "Dividir párrafos largos (máx 80-100 palabras)"
            )
        else:
