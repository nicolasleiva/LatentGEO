...
class SchemaOptimizerService:
    ...
    async def generate_schema(
        ...
        return {
            ...
            'page_type': page_type,
            'is_valid': is_valid,
            'implementation_code': SchemaOptimizerService._generate_implementation_code(schema)
        }
    ...
    def _detect_page_type(soup: BeautifulSoup, url: str) -> str:
        ...
        # FAQ Page
        if ('pregunta' in text_content or 'faq' in text_content or 
            len(soup.find_all(['dt', 'dd'])) > 3):
            return 'FAQPage'
        ...
        # Product
        if ('precio' in text_content or 'comprar' in text_content or 
            soup.find(class_=lambda x: x and 'price' in x.lower())):
            return 'Product'
        
        # HowTo
        if ('cómo' in text_content or 'paso' in text_content or
            len(soup.find_all(['ol', 'ul'])) > 2):
            return 'HowTo'
...
{schema_json}
    ...
    def generate_multiple_schemas(
        ...
        org_schema = SchemaOptimizerService._generate_base_schema(soup, url, 'Organization')
        ...
        if page_type != 'Organization':
            specific_schema = SchemaOptimizerService._generate_base_schema(soup, url, page_type)
