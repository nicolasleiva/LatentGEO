...
class BacklinksService:
    ...
    @staticmethod
    def generate_backlinks_from_audit(
        target_audit: Dict[str, Any], url: str
    ) -> Dict[str, Any]:
        """
        ...
        if settings.ENVIRONMENT == "production" or settings.STRICT_CONFIG:
            raise RuntimeError(
                "BacklinksService is a stub in production. Use BacklinkService."
            )
        try:
            ...
            referring_domains = len(
                set(b["source_url"].split("/")[2] for b in backlinks)
            )
            avg_da = (
                sum(b["domain_authority"] for b in backlinks) / total_backlinks
            ...
            return {"total_backlinks": 0, "referring_domains": 0, "top_backlinks": []}
