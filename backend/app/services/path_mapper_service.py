...
class PathMapperService:
    ...
    @staticmethod
    def map_urls_to_files(
        fix_plan: List[Dict], repo_files: List[ContentFile]
    ) -> Dict[str, str]:
        """
        ...
        url_paths_to_fix = {
            ...
            if "page_path" in item and item["page_path"] != "ALL_PAGES"
        }
        ...
        for url_path in url_paths_to_fix:
            ...
            for file in repo_files:
                ...
                base_name = (
                    base_name.replace("src/pages/", "").replace("app/", "").strip("/")
                )
