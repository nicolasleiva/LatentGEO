    ...
    def __init__(self):
        ...
        if (
            ...
            if not GOOGLE_ADS_AVAILABLE:
                logger.warning(
                    "google-ads package not installed. Real data will be unavailable."
                )
            else:
                logger.warning(
                    "Google Ads credentials not found. Real data will be unavailable."
                )

    def get_keyword_metrics(
        self, keywords: List[str], location_id: str = "2840", language_id: str = "1000"
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            request.geo_target_constants = [
                self.client.get_service("GoogleAdsService").geo_target_constant_path(
                    location_id
                )
            ]
